...
public:
  void solve(double, double, unsigned int);
};

void RichardsonExtrapolation::solve(double x0, double h, unsigned int order) {
  unsigned int n, k; /* max rows and cols in table */
  k = (order - 2) / 2;
  n = k;
...
int main(int argc, char **argv) {
  ...
  RichardsonExtrapolation solver;
  solver.solve(x0, h, order);
}
#endif